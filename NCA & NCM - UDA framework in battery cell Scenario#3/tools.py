...
def get_Tsp(source_dataset: pd.DataFrame, verbose= False):
    ...
    for cell_name in np.unique(source_dataset["Cell"]):
        cell_curve= source_dataset[source_dataset["Cell"]== cell_name]
        cell_curve_fit_knee_point= cell_curve[cell_curve["Capacity"]>= 0.80]
        x_data= np.arange(1, len(cell_curve_fit_knee_point["Capacity"])+ 1)
        y_data= np.array(cell_curve_fit_knee_point["Capacity"])
        ...
        if knee_point<= 0 or knee_point>= len(y_data):
            ...
            if verbose== True:
                ...
                plt.plot(x_data, y_data, label= cell_name, color= "darkblue")
                plt.plot(x_data, bacon_watts_knee_point(x_data, opt_alpha0, opt_alpha1, opt_alpha2, opt_x1, scaler), color= "darkred")
                plt.scatter(knee_point, y_data[knee_point], label= "knee point: cycle - "+ str(knee_point), color= "black", s= 50, marker= "o")
            ...
            # print("PSP capacity persentage: ", y_data[knee_point])
            # print("PSP capacity: ", y_data[knee_point]* 3500)
...
def data_smoothing(dataset: pd.DataFrame, lowess_tau: float, verbose= False):
    ...
    if verbose== True:
        if len(np.unique(dataset["Cell"]))> 10:
            plt.figure(figsize= (6, 30)); index= 1
        elif len(np.unique(dataset["Cell"]))>= 5:
            plt.figure(figsize= (6, 15)); index= 1
        else:
            plt.figure(figsize= (6, 3)); index= 1
    for cells in np.unique(dataset["Cell"]):
        sub_dataset, capacity= dataset[dataset["Cell"]== cells], dataset[dataset["Cell"]== cells]["Capacity"]
        smoothed_y= sm.nonparametric.lowess(capacity, np.arange(0, len(capacity)), frac= lowess_tau)[:, 1]
        sub_dataset["Capacity"]= smoothed_y
        if verbose== True:
            plt.gca().set_title(cells)
            plt.subplot(len(np.unique(dataset["Cell"])), 1, index)
            plt.plot(np.arange(0, len(capacity)), capacity, color= "darkblue")
...
def sliding_window_approach(dataset, m):
    cells= np.unique(dataset["Cell"]); sliding_window_dataset= pd.DataFrame()
    for cell_i in range(0, len(cells)):
        cell_dataset= dataset[dataset["Cell"]== cells[cell_i]]
        cell_name, cell_capacity= cell_dataset["Cell"], np.array(cell_dataset["Capacity"])
...
def data_normalization(dataset: pd.DataFrame):
    ...
    dataset["Capacity"]= normalizer.fit_transform(dataset[["Capacity"]])
