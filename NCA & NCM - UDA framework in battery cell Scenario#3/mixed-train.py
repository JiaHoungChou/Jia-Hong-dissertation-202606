...
if __name__== "__main__":
    ...
    total_loss_ls= []; predictor_loss_ls= [];  epoch_time_ls= []
    ...
    for e in range(1, epochs+ 1):
        s_time= time.time()
        Encoder.train(); Predictor.train(); x= []; batch_mean_square_error_loss= []
        ...
        if np.mean(batch_mean_square_error_loss)< early_stopping["best_loss"]:
            early_stopping["best_loss"]= np.mean(batch_mean_square_error_loss)
            early_stopping["best_epoch"]= e
            early_stopping["count"]= 0
            torch.save(Encoder, os.path.join(os.getcwd(), "encoder-mixed-train.pth")); torch.save(Predictor, os.path.join(os.getcwd(), "predictor-mixed-train.pth"))
        else:
