...
class EnLSTM(nn.Module):
    def __init__(self, in_dim, head_size, num_heads, model_mode= "train"):
        ...
        self.lstm= nn.LSTM(input_size= self.in_dim, hidden_size= self.head_size, num_layers=1, bias= True, batch_first= True, dropout= 0.0, bidirectional=False, proj_size=0)
...
class DnnPredictor(nn.Module):
    ...
    def gussain_noise_layer(self, input, mean= 0.0, standard_deviaton= 0.008):
        if self.is_train:
            noise= input.data.new(input.size()).normal_(mean, standard_deviaton)
            return noise+ input
        return input
...
if __name__== "__main__":
    ...
    for target_cell_name in target_cell_name_ls:
        ...
        plt.scatter(Tsp, Tsp_capacity, color= "darkorange", marker= "o", s= 100, label= "Tsp")
