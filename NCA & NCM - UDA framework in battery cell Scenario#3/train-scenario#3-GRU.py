...
class EnLSTM(nn.Module):
    def __init__(self, in_dim, head_size, num_heads, model_mode= "train"):
        ...
        self.lstm= nn.GRU(input_size= self.in_dim, hidden_size= self.head_size, num_layers=1, bias= True, batch_first= True, dropout= 0.0, bidirectional=False)
...
class DnnPredictor(nn.Module):
    ...
    def gussain_noise_layer(self, input, mean= 0.0, standard_deviaton= 0.011):
        if self.is_train:
            noise= input.data.new(input.size()).normal_(mean, standard_deviaton)
            return noise+ input
        return input
