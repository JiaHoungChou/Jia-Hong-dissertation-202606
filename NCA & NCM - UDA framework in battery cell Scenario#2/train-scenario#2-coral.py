...
def coral(source, target):
    ...
    assert source.size(1) == target.size(1), "Feature dimension must match"
    d = source.size(1)
...
if __name__== "__main__":
    ...
    Tsp, Tsp_capacity= get_Tsp(source_full_cell_dataset, verbose= True)
    ...
    lambda_D= 0.8; lambda_T= 0.1; lambda_S= 0.1; lambda_P= 0.2
    ...
    for e in range(1, epochs+ 1):
        ...
        for target_cell_name in target_cell_name_ls:
            for source_cell_name in np.random.choice(source_cell_name_ls, len(target_cell_name_ls), replace= False):
                ...
                domain_loss = coral(Sdomain_y[: min_len], Tdomain_y[: min_len])
