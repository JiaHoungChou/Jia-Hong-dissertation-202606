...
class EnLSTM(nn.Module):
    ...
    def gussain_noise_layer(self, input, mean= 0.0, standard_deviaton= 0.001):
        if self.is_train:
...
class DnnPredictor(nn.Module):
    ...
    def gussain_noise_layer(self, input, mean= 0.0, standard_deviaton= 0.01):
        if self.is_train:
            noise= input.data.new(input.size()).normal_(mean, standard_deviaton)
            return noise+ input
        return input
...
def partial_before_dataset_generator(dataset: pd.DataFrame, Tsp: float):
    ...
    for cell_name in cell_ls:
        ...
        cell_data= cell_data.iloc[: len(cell_data[cell_data["Capacity"]>= Tsp_capacity]), :]
...
def gaussian_kernel(x, y, sigmas=None):
    ...
    xx = x.unsqueeze(1) 
    yy = y.unsqueeze(0) 
    l2_dist = ((xx - yy) ** 2).sum(dim=2)
    kernels = 0.0
    for sigma in sigmas:
        gamma = 1.0 / (2.0 * sigma * sigma)
        kernels = kernels + torch.exp(-gamma * l2_dist)
    return kernels

def mmd(source, target, sigmas=None):
...
if __name__== "__main__":
    ...
    SOURCE_DOMAIN= "nca-25"; TARGET_DOMAIN= "ncm-25"
    SOURCE_CELLS= ["NCA cell#11", "NCA cell#18", "NCA cell#16", "NCA cell#10", "NCA cell#19", "NCA cell#07", "NCA cell#13", "NCA cell#02"]
    SOURCE_CELLS= [cell.split(" ")[0]+ " "+ "Cell%2d"%(int(cell.split("#")[-1])) for cell in SOURCE_CELLS]
    TARGET_CELLS= ["NCM cell#08", "NCM cell#16", "NCM cell#05"]
    ...
    Tsp, Tsp_capacity= get_Tsp(source_full_cell_dataset, verbose= False)
    ...
    for e in range(1, epochs+ 1):
        ...
        for target_cell_name in target_cell_name_ls:
            for source_cell_name in np.random.choice(source_cell_name_ls, len(target_cell_name_ls), replace= False):
                ...
                min_len = min(Sdomain_y.numel(), Tdomain_y.numel())
                domain_loss = mmd(Sdomain_y[: min_len], Tdomain_y[: min_len])
        ...
        e_time= time.time()
        epoch_time_ls.append(e_time- s_time)
        ...
        if e% 1== 0:
            ...
            if Tpartial_valid_loss< early_stopping["best_loss"]:
                ...
                torch.save(Encoder, os.path.join(os.getcwd(), "encoder-mmd.pth")); torch.save(Predictor, os.path.join(os.getcwd(), "predictor-mmd.pth"))
            else:
            ...
            if early_stopping["count"]== 10:
                ...
                break
    print("time consuming: %f"%(np.mean(epoch_time_ls)))
    Encoder.eval(); Predictor.eval()
    monte_carlo= 30
    target_full_cell_dataset= real_csvfiles_in_folder(os.path.join(dataset_folder, TARGET_DOMAIN.split("-")[0], TARGET_DOMAIN.split("-")[1]))
    target_full_cell_dataset= data_smoothing(dataset= target_full_cell_dataset, lowess_tau= 1/10, verbose= False)
