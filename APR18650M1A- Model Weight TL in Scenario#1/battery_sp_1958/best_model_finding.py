...
def rul_prediction(generated_cycle, ML_train_x, ML_train_y, ML_normalizer_x, ML_normalizer_y, DL_train_x, DL_train_y, ML_model, DL_model, eol_value, actual_eol, sp_cycle):
        ...
        try:
            composed_SOH_valid= np.array(composed_SOH_valid).ravel()

            return composed_SOH_valid
...
if __name__== "__main__":
    ...
    df_Battery_1= df[["Cycle", "#1_SOH"]].dropna()

    cycle= np.array(df_Battery_1["Cycle"])

    x_data= np.arange(1, len(df_Battery_1)+ 1)
    y_data= np.array(df_Battery_1["#1_SOH"])
    ...
    print(mdoel_weight_saving_path)
    ...
    actual_eol= 2158; actual_eol_soh= 0.8; distance_list= []
    actual_rul= actual_eol- Training_Cycle_Index[1]+ 1

    print(actual_rul)

    for index in range(0, len(DL_model_check_point_list)):
        print(index)
        check_point= DL_model_check_point_list[index]

        seq_len_param= int(check_point.split("_")[4])
        hidden_size_param= int(check_point.split("_")[7])
        batch_size_param= int(check_point.split("_")[-1])

        print("seq_len_param: ", seq_len_param)
        print("hidden_size_param: ", hidden_size_param)
        print("batch_size_param: ", batch_size_param)
        ...
        composed_SOH= rul_prediction(generated_cycle= generated_cycle,
        ...
        distance= round(dtw.distance(composed_SOH, test_y), 4)

        distance_list.append(distance)
        
        predicted_rul= len(composed_SOH)
        predicted_eol= Testing_Cycle_Index[0]+ 1+ predicted_rul

        plt.figure(figsize= (13, 4))
        plt.title("online rul prediction for battery 1.", fontsize= 20, fontweight= "bold")
        ...
        plt.plot(np.arange(Testing_Cycle_Index[0]+ 1, predicted_eol), composed_SOH, label= "SOH online prediction", color= "red", alpha= 0.5)
        plt.hlines(0.8, 0, actual_eol, color= "red", label= "EOL (SOH= 0.8)", ls= "--")
        plt.ylabel("SOH", fontsize= 20)
        plt.xlim(0, predicted_eol)
        plt.hlines(0.8, 0, predicted_eol, color= "red", label= "EOL (SOH= 0.8)", ls= "--")
        ...
        print("--------> pred RUL:%4d, pred EOL:%4d"%(predicted_rul, predicted_eol))
        print("===============================================================")
        print("--------> Erro RUL:%4d, Erro EOL: %4d"%(actual_rul- predicted_rul, actual_eol- predicted_eol), "\n\n\n")

    print("minimum DTW distance index: ", np.where(np.array(distance_list)== np.min(distance_list))[0])