...
def prediction_generation(generated_cycle, ML_train_x, ML_train_y, ML_normalizer_x, ML_normalizer_y, DL_train_x, DL_train_y, ML_model, DL_model, eol_value):
    ML_train_x= ML_normalizer_x.transform(ML_train_x)
    ...
    try:
        return np.array(composed_SOH_valid).ravel()
...
if __name__== "__main__":
    ...
    mdoel_weight_saving_path=  r"C:\Users\MA201-01\Desktop\Group1\battery_sp_1958"

    file_path= r"C:\Users\MA201-01\Desktop\Group1\battery_sp_1958\Battery_1-_2.csv"

    with open(file_path, "r") as file:
        df= pd.read_csv(file, index_col= False)

    df_Battery_2= df[["Cycle", "#2_SOH"]].dropna()

    cycle= np.array(df_Battery_2["Cycle"])

    x_data= np.arange(1, len(df_Battery_2)+ 1)
    y_data= np.array(df_Battery_2["#2_SOH"])
    ...
    Training_Cycle_Index= [knee_onset_double_bacon_watts, 1958]
    Testing_Cycle_Index=  [1958+ 1, -1]
    ...
    ############### RUL prediction ###############
    model_libaray= r"C:\Users\MA201-01\Desktop\Group1\battery_sp_1958\model_check_point"
    ...
    selected_param= {"seq_len": 34, "hidden_size": 102, "batch_size": 10}
    ...
    rul_prediction_storage= []; predicted_eol= []
    for i in range(0, n_generation):
        ...
        composed_SOH= prediction_generation(generated_cycle= generated_cycle,
                                            ML_train_x= x_train_IMFs, 
                                            ML_train_y= y_train_IMFs, 
                                            ML_normalizer_x= ML_normalizer_x, 
                                            ML_normalizer_y= ML_normalizer_y, 
                                            DL_train_x= x_train_Residual_in,
                                            DL_train_y= y_train_Residual_in, 
                                            ML_model= pre_traned_ML_model, 
                                            DL_model= pre_traned_DL_model, 
                                            eol_value= actual_eol_soh)
        
        rul_prediction_storage.append(composed_SOH)
        predicted_eol.append(len(composed_SOH))

    plt.figure(figsize= (13, 4))
    plt.plot(np.arange(0, Training_Cycle_Index[1]), y_data[: Training_Cycle_Index[1]], label= "actual battery #2 (known)", color= "black")
    plt.plot(np.arange(Testing_Cycle_Index[0]+ 1, len(y_data)), test_y, label= "actual battery #2 (unknown)", color= "blue")
    
    for i in range(0, len(rul_prediction_storage)):
        if i== 0:
            plt.plot(np.arange(Testing_Cycle_Index[0]+ 1, Testing_Cycle_Index[0]+ 1+ len(rul_prediction_storage[i])), rul_prediction_storage[i], label= "prediction uncertainty", color= "red", alpha= 0.5)
        else:
            plt.plot(np.arange(Testing_Cycle_Index[0]+ 1, Testing_Cycle_Index[0]+ 1+ len(rul_prediction_storage[i])), rul_prediction_storage[i], color= "red", alpha= 0.5)
    
    # plt.scatter(knee_onset_double_bacon_watts, y_data[knee_onset_double_bacon_watts], label= "knee onset (double Bacon Watts)", color= "darkred", s= 100, marker= "o")
    plt.scatter(Training_Cycle_Index[1], y_data[Training_Cycle_Index[1]], label= "Sp (#1959)", color= "black", s= 100, marker= "o")
    plt.ylabel("SOH", fontsize= 20)
    plt.xlim(0, Testing_Cycle_Index[0]+ 1+ np.max([np.max(predicted_eol), len(y_data[Training_Cycle_Index[1]: ])]))
    plt.hlines(0.8, 0, Testing_Cycle_Index[0]+ 1+ np.max([np.max(predicted_eol), len(y_data[Training_Cycle_Index[1]: ])]), color= "red", label= "EOL (SOH= 0.8)", ls= "--")
    ...
    rul_lower_bound= round(np.mean(predicted_eol)- 3* np.std(predicted_eol), 4)
    rul_upper_bound= round(np.mean(predicted_eol)+ 3* np.std(predicted_eol), 4)
    
    print("--------> RUL prediction interval: Upper_bound: %4.4f - Lower_bound: %4.4f"%(rul_upper_bound, rul_lower_bound))
    print("--------> EOL prediction interval: Upper_bound: %4.4f - Lower_bound: %4.4f"%(Testing_Cycle_Index[0]+ 1+ rul_upper_bound, Testing_Cycle_Index[0]+ 1+ rul_lower_bound))