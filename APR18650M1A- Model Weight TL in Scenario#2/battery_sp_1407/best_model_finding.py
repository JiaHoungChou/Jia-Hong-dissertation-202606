...
if __name__== "__main__":
    ...
    df_Battery_3= df_Battery_3[df_Battery_3["#4_SOH"] >= 0.80]
    ...
    est_rul= []; est_eol= []; est_soh= []; est_cycle= []; distance_list= []
    for index in range(0, len(DL_model_check_point_list)):
        ...
        est_rul.append(pred_rul); est_eol.append(pred_eol); est_soh.append(composed_SOH); est_cycle.append(pred_cycle)

    
        plt.figure(figsize= (13, 4))
        plt.plot(np.arange(Testing_Cycle_Index[0]+ 1, len(y_data)), test_y, label= "battery_2 (unknown trend curve)")
        plt.plot(pred_cycle, composed_SOH, label= "SOH online prediction")
        plt.xlim(Testing_Cycle_Index[0], np.max([Testing_Cycle_Index[0]+ len(composed_SOH), Testing_Cycle_Index[0]+ len(test_y)]))
        plt.legend(loc= "best")
        plt.show()
        ...
        print("===============================================================")
        print("--------> Erro RUL:%4.2f, Erro EOL: %4.2f"%(round(actual_eol- sp- np.mean(est_rul), 4), round(actual_eol- np.mean(est_eol), 4)), "\n\n\n")

    
    print("best model index: ", np.where(np.array(distance_list)== np.min(distance_list))[0][0])
    print("distance: ", np.min(distance_list))